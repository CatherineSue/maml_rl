...
from rllab.envs.normalized_env import BabyModeWrapper
...
for fast_learning_rate in fast_learning_rates:
    for learning_rate in learning_rates:
        for bas in baselines:
            ...
            tasks =[
                {'goal': g, 'obj_init_pos':[0, 0.6, 0.02], 'obj_init_angle': 0.3, 'type':'reach'}
                for i, g in enumerate(goals)
            ]

            push_env = BabyModeWrapper(SawyerReachPushPickPlace6DOFEnv(
                random_init=False,
                multitask=False,
                obs_type='plain',
                if_render=False,
            ...
            run_experiment_lite(
                ...
                seed=1,
                exp_prefix='maml_rl_trpo_baby_{}_{}'.format(TASKNAME, timestamp),
